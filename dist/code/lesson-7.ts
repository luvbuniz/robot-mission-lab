...
const STOP_CM = 20 // Teacher calibrates with low-speed tests.
input.onButtonPressed(Button.A, function () {
    ...
    while (active()) {
        ...
        // Measurement can yield to another fiber; recheck B stop before motors.
        if (!active()) break
        if (distance <= 0 || distance <= STOP_CM) {
            finish() // Latched stop: this A-triggered trial cannot restart.
            break
        }
        Kitronik_Move_Motor.move(Kitronik_Move_Motor.DriveDirections.Forward, SPEED)
        if (!waitActive(50)) break
    }
    finish()
    busy = false
})
